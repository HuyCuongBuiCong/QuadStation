...
public class BT extends Communication {
	...
	@Override
	public void Enable() {
		Toast.makeText(context, "Starting Bluetooth", Toast.LENGTH_SHORT)
				.show();
		if (D)
			Log.d(TAG, "+++ Enable BT +++");
	...
	public void Connect(String address, int speed) {
		...
		if (mBluetoothAdapter.isEnabled()) {
			try {
				...
				try {
					...
					Log.e(TAG,
							...
							e2);
					Toast.makeText(context, "Connection failure",
							Toast.LENGTH_LONG).show();
				}
			}
			...
			try {
				...
				Toast.makeText(context, "Stream creation failed",
						Toast.LENGTH_LONG).show();
			}
		}
	...
	public void Disable() {
		try {
			...
			Toast.makeText(context, "Can't dissable BT", Toast.LENGTH_LONG)
					.show();
		}
	}
	...
	public void CloseSocket() {
		if (outStream != null) {
			try {
				...
				Toast.makeText(context, "Unable to close socket",
						Toast.LENGTH_LONG).show();
			}
		}
		...
		try {
			...
			Toast.makeText(context, "Unable to close socket", Toast.LENGTH_LONG)
					.show();
		}
