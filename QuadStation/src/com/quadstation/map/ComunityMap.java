...
import com.quadstation.helper.MyBase64;
...
public class ComunityMap extends AsyncTask {
	...
	public void send(double selectedLatitude, double selectedLongitude,
			String nick, String description) {

		String data = format.format(selectedLatitude / 1e6) + ";"
				+ format.format(selectedLongitude / 1e6) + ";"
				+ MyBase64.encode(nick.getBytes()) + ";"
				+ MyBase64.encode(description.getBytes());
		try {
			execute(data);
			Log.d("aaa", "Added to comunity map");
			Toast.makeText(context, "Point added to map", Toast.LENGTH_LONG)
					.show();
		} catch (Exception e) {
