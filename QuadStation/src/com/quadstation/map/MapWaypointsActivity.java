...
public class MapWaypointsActivity extends SherlockFragmentActivity {
	...
	private Runnable update = new Runnable() {
		...
		public void run() {
			...
			LatLng copterPositionLatLng = new LatLng(app.mw.GPS_latitude
					/ Math.pow(10, 7), app.mw.GPS_longitude / Math.pow(10, 7));

			// Map centering
			if (MoveMap && centerStep < System.currentTimeMillis()) {
				if (app.mw.GPS_fix == 1) {
					mapHelperClass.map.animateCamera(CameraUpdateFactory
							.newCameraPosition(new CameraPosition(
									copterPositionLatLng, app.MapZoomLevel, 0,
									app.mw.head)));
				} else {
					mapHelperClass.map.animateCamera(CameraUpdateFactory
							.newCameraPosition(new CameraPosition(new LatLng(
									app.sensors.PhoneAltitude,
									app.sensors.PhoneLatitude),
									app.MapZoomLevel, 0, 0)));
				}
				centerStep = System.currentTimeMillis() + app.MapCenterPeriod
						* 1000;
			}
			...
			mapHelperClass.SetCopterLocation(copterPositionLatLng, app.mw.head,
					app.mw.alt);
			Log.d("MapWayPoint", "startDrawPath");
			mapHelperClass.DrawFlightPath(copterPositionLatLng);
			mapHelperClass.PositionHoldMarker.setPosition(app.mw.Waypoints[16]
					.Lat_Lng());
			mapHelperClass.HomeMarker
					.setPosition(app.mw.Waypoints[0].Lat_Lng());

			if (CurrentWaypointNumber >= 0) {
				distanceToNextWaypoint = (float) MapHelperClass.gps2m(
						copterPositionLatLng.latitude,
						copterPositionLatLng.longitude,
						mapHelperClass.markers.get(CurrentWaypointNumber)
	...
	void DisplayInfo() {
		...
		if (CurrentWaypointNumber >= 0) {
			TVInfoMap.setText("WP#" + String.valueOf(CurrentWaypointNumber + 1)
					+ "\n");
			TVInfoMap.append("To WP#"
					+ String.valueOf(CurrentWaypointNumber + 1) + ":"
					+ format.format(distanceToNextWaypoint) + "m" + "\n");
		}
		TVInfoMap.append("Alt:" + format.format(app.mw.alt) + "m" + "\n");
		TVInfoMap.append("Bat:" + format.format(app.mw.bytevbat / 10.0) + "V"
				+ "\n");
		TVInfoMap.append("Power:" + String.valueOf(app.mw.pMeterSum) + "/"
				+ String.valueOf(app.mw.intPowerTrigger) + "\n");
		TVInfoMap.append("Sat:" + String.valueOf(app.mw.GPS_numSat) + "\n");
		TVInfoMap.append("Head:" + String.valueOf((int) app.mw.head) + "°"
				+ "\n");
		TVInfoMap.append("Speed:" + String.valueOf((int) app.mw.GPS_speed)
				+ "m/s" + "\n");
		TVInfoMap.append("ToHome:" + format.format(app.mw.GPS_distanceToHome)
				+ "m" + "\n");
		TVInfoMap.append("DirToHome:"
				+ String.valueOf((int) app.mw.GPS_directionToHome + "°"));
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (GooglePlayServicesUtil
			...
			Toast.makeText(this, getString(R.string.GooglePlayServiecesError),
				...
				.setOnMapLongClickListener(new OnMapLongClickListener() {
					...
					public void onMapLongClick(LatLng point) {
						...
						Toast.makeText(getApplicationContext(),
								String.valueOf(point.latitude * 1e6),
		...
		mapHelperClass.map.animateCamera(CameraUpdateFactory
				.newCameraPosition(new CameraPosition(new LatLng(0, 0),
						mapHelperClass.map.getMinZoomLevel(), 0, 0)));
	...
	void SetNextWaypoint(boolean previous) {
		...
		if (CurrentWaypointNumber >= mapHelperClass.markers.size()
		...
		int alt = 0;
		app.mw.SendRequestMSP_SET_WP(new Waypoint(16,
				(int) (mapHelperClass.markers.get(CurrentWaypointNumber)
						.getPosition().latitude * 1e6 * 10),
				(int) (mapHelperClass.markers.get(CurrentWaypointNumber)
						.getPosition().longitude * 1e6 * 10), alt, 0, 0, 0));
		mapHelperClass.CurrentWPCircle.setCenter(mapHelperClass.markers.get(
				CurrentWaypointNumber).getPosition());
		...
		mapHelperClass.markers.get(CurrentWaypointNumber).setIcon(
				BitmapDescriptorFactory
						.defaultMarker(BitmapDescriptorFactory.HUE_ORANGE));

		if (app.D) {
			app.mw.Waypoints[16].Lat = (int) (mapHelperClass.markers.get(
					CurrentWaypointNumber).getPosition().latitude * 1e6 * 10);
			app.mw.Waypoints[16].Lon = (int) (mapHelperClass.markers.get(
					CurrentWaypointNumber).getPosition().longitude * 1e6 * 10);
		}
	}
	...
	void AddCircle(double diameterInMeters, int howManyPoints) {
		...
		for (int i = 0; i < howManyPoints; i++) {
			mapHelperClass.AddMarker((MapHelperClass
					.GetPointGivenRadialAndDistance(
							mapHelperClass.markers.get(0).getPosition(),
							diameterInMeters, azimuth)));
			azimuth += x;
		}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if (item.getItemId() == R.id.MenuMapWaypointMoveMap) {
			...
			if (MoveMap)
				Toast.makeText(getApplicationContext(),
						getString(R.string.MoveMapEnable), Toast.LENGTH_SHORT)
						.show();
			else
				Toast.makeText(getApplicationContext(),
						getString(R.string.MoveMapDissable), Toast.LENGTH_SHORT)
						.show();
			return true;
		}
	...
	private final class CircleOptionsActionModeMenu implements
			ActionMode.Callback {
		@Override
		public boolean onCreateActionMode(ActionMode mode, Menu menu) {

			menu.add(Menu.NONE, 1, Menu.NONE, getString(R.string.Bigger))
					.setShowAsAction(MenuItem.SHOW_AS_ACTION_ALWAYS);
			menu.add(Menu.NONE, 2, Menu.NONE, getString(R.string.Smaller))
					.setShowAsAction(MenuItem.SHOW_AS_ACTION_ALWAYS);
			menu.add(Menu.NONE, 3, Menu.NONE, getString(R.string.AddWP))
					.setShowAsAction(MenuItem.SHOW_AS_ACTION_IF_ROOM);
			menu.add(Menu.NONE, 4, Menu.NONE, getString(R.string.RemWP))
					.setShowAsAction(MenuItem.SHOW_AS_ACTION_IF_ROOM);
		...
		public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
			if (item.getItemId() == 1) {
				...
				while (mapHelperClass.markers.size() > 1) {
					mapHelperClass
							.RemoveMarker(mapHelperClass.markers.size() - 1);
				}
