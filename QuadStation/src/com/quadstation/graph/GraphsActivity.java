...
import com.quadstation.graph.GraphView;
import com.quadstation.graph.GraphView.*;
...
public class GraphsActivity extends SherlockActivity {
			...
			if (!pause) {
				...
				if (CurentPosition == NextLimit) {
					for (GraphViewSeries s : series) {
						s.resetData(new GraphViewData[] { new GraphViewData(
								CurentPosition, 0) });
					}
				...
				for (GraphViewSeries s : series) {
					...
					if (s.description.equals(app.ACCROLL))
						s.appendData(new GraphViewData(CurentPosition,
								app.mw.ax), true);
	...
	void graphInit() {
		...
		if (gr.contains(app.ACCROLL))
			series.add(new GraphViewSeries(app.ACCROLL,
					new GraphViewSeriesStyle(Color.RED, 3),
					new GraphViewData[] { new GraphViewData(0, 0) }));

		if (gr.contains(app.ACCPITCH))
			series.add(new GraphViewSeries(app.ACCPITCH,
					new GraphViewSeriesStyle(Color.GREEN, 3),
					new GraphViewData[] { new GraphViewData(0, 0) }));
		if (gr.contains(app.ACCZ))
			series.add(new GraphViewSeries(app.ACCZ, new GraphViewSeriesStyle(
					Color.BLUE, 3), new GraphViewData[] { new GraphViewData(0,
					0) }));

		if (gr.contains(app.GYROROLL))
			series.add(new GraphViewSeries(app.GYROROLL,
					new GraphViewSeriesStyle(Color.rgb(196, 201, 0), 3),
					new GraphViewData[] { new GraphViewData(0, 0) }));
		if (gr.contains(app.GYROPITCH))
			series.add(new GraphViewSeries(app.GYROPITCH,
					new GraphViewSeriesStyle(Color.rgb(0, 255, 255), 3),
					new GraphViewData[] { new GraphViewData(0, 0) }));
		if (gr.contains(app.GYROYAW))
			series.add(new GraphViewSeries(app.GYROYAW,
					new GraphViewSeriesStyle(Color.rgb(255, 0, 255), 3),
					new GraphViewData[] { new GraphViewData(0, 0) }));

		if (gr.contains(app.MAGROLL))
			series.add(new GraphViewSeries(app.MAGROLL,
					new GraphViewSeriesStyle(Color.rgb(52, 101, 144), 3),
					new GraphViewData[] { new GraphViewData(0, 0) }));
		if (gr.contains(app.MAGPITCH))
			series.add(new GraphViewSeries(app.MAGPITCH,
					new GraphViewSeriesStyle(Color.rgb(98, 51, 149), 3),
					new GraphViewData[] { new GraphViewData(0, 0) }));
		if (gr.contains(app.MAGYAW))
			series.add(new GraphViewSeries(app.MAGYAW,
					new GraphViewSeriesStyle(Color.rgb(150, 100, 49), 3),
					new GraphViewData[] { new GraphViewData(0, 0) }));

		if (gr.contains(app.ALT))
			series.add(new GraphViewSeries(app.ALT, new GraphViewSeriesStyle(
					Color.rgb(130, 122, 125), 3),
					new GraphViewData[] { new GraphViewData(0, 0) }));
		if (gr.contains(app.HEAD))
			series.add(new GraphViewSeries(app.HEAD, new GraphViewSeriesStyle(
					Color.rgb(255, 226, 124), 3),
					new GraphViewData[] { new GraphViewData(0, 0) }));

		if (gr.contains(app.DEBUG1))
			series.add(new GraphViewSeries(app.DEBUG1,
					new GraphViewSeriesStyle(Color.rgb(200, 50, 0), 3),
					new GraphViewData[] { new GraphViewData(0, 0) }));

		if (gr.contains(app.DEBUG2))
			series.add(new GraphViewSeries(app.DEBUG2,
					new GraphViewSeriesStyle(Color.rgb(0, 200, 50), 3),
					new GraphViewData[] { new GraphViewData(0, 0) }));

		if (gr.contains(app.DEBUG3))
			series.add(new GraphViewSeries(app.DEBUG3,
					new GraphViewSeriesStyle(Color.rgb(50, 0, 200), 3),
					new GraphViewData[] { new GraphViewData(0, 0) }));

		if (gr.contains(app.DEBUG4))
			series.add(new GraphViewSeries(app.DEBUG4,
					new GraphViewSeriesStyle(Color.rgb(150, 100, 50), 3),
					new GraphViewData[] { new GraphViewData(0, 0) }));
