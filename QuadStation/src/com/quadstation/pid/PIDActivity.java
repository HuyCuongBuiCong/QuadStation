...
import com.quadstation.helper.CustomInputDialog;
...
public class PIDActivity extends SherlockActivity {
	...
	void ShareIt() {
		...
		try {
			...
			Log.d("aaa",
					"File to send:" + Environment.getExternalStorageDirectory()
							+ "/MultiWiiLogs/"
	...
	public void SetOnClick(View v) {
		...
		confRC_RATE = Float.parseFloat(RcRate.getText().toString()
				.replace(",", "."));
		confRC_EXPO = Float.parseFloat(RcExpo.getText().toString()
				.replace(",", "."));
		rollPitchRate = Float.parseFloat(RollPitchRate.getText().toString()
				.replace(",", "."));
		yawRate = Float.parseFloat(YawRate.getText().toString()
				.replace(",", "."));
		dynamic_THR_PID = Float.parseFloat(ThrottleRate.getText().toString()
				.replace(",", "."));
		throttle_MID = Float.parseFloat(ThrottleMid.getText().toString()
				.replace(",", "."));
		throttle_EXPO = Float.parseFloat(ThrottleExpo.getText().toString()
				.replace(",", "."));
		...
		if (v != null) {
			app.mw.SendRequestMSP_SET_PID(confRC_RATE, confRC_EXPO,
					rollPitchRate, yawRate, dynamic_THR_PID, throttle_MID,
					throttle_EXPO, P, I, D);
					...
					.setPositiveButton(getString(R.string.Yes),
							new DialogInterface.OnClickListener() {
								...
								public void onClick(DialogInterface dialog,
									...
									app.mw.SendRequestMSP_SET_PID(confRC_RATE,
											confRC_EXPO, rollPitchRate,
											yawRate, dynamic_THR_PID,
											throttle_MID, throttle_EXPO, P, I,
											D);
									app.mw.SendRequestMSP_EEPROM_WRITE();
	...
	public void LoadProfilePIDOnClick(View v) {
		try {
			if (spinnerProfile.getCount() > 0)
				readFromXML("/MultiWiiLogs/"
						+ spinnerProfile.getSelectedItem().toString() + ".mwi");
	...
	private void readFromXML(String fileName)
		...
		RollPitchRate.setText(String.format("%.2f",
				Float.valueOf(properties.getProperty("rc.rollpitch.rate"))));
		RollPitchRate2.setText(String.format("%.2f",
				Float.valueOf(properties.getProperty("rc.rollpitch.rate"))));

		YawRate.setText(String.format("%.2f",
				Float.valueOf(properties.getProperty("rc.yaw.rate"))));

		ThrottleMid.setText(String.format("%.2f",
				Float.valueOf(properties.getProperty("rc.throttle.mid"))));
		ThrottleExpo.setText(String.format("%.2f",
				Float.valueOf(properties.getProperty("rc.throttle.expo"))));

		RcRate.setText(String.format("%.2f",
				Float.valueOf(properties.getProperty("rc.rate"))));
		RcExpo.setText(String.format("%.2f",
				Float.valueOf(properties.getProperty("rc.expo"))));

		ThrottleRate.setText(String.format("%.2f",
				Float.valueOf(properties.getProperty("rc.throttle.rate"))));
	...
	private void ShowData() {
		...
		RcRate.setText(String
				.format("%.2f", (float) app.mw.byteRC_RATE / 100.0));
		RcExpo.setText(String
				.format("%.2f", (float) app.mw.byteRC_EXPO / 100.0));

		ThrottleRate.setText(String.format("%.2f",
				(float) app.mw.byteDynThrPID / 100.0));
	}
	...
	private void SaveToXml(String fileName)
		...
		for (int i = 0; i < app.mw.PIDITEMS; i++) {
			properties.setProperty("pid." + i + ".p", String.valueOf(P[i])
					.replace(",", "."));
			properties.setProperty("pid." + i + ".i", String.valueOf(I[i])
					.replace(",", "."));
			properties.setProperty("pid." + i + ".d", String.valueOf(D[i])
					.replace(",", "."));
		}

		properties.setProperty("rc.rollpitch.rate", RollPitchRate.getText()
				.toString().replace(",", "."));
		properties.setProperty("rc.yaw.rate", YawRate.getText().toString()
				.replace(",", "."));
		properties.setProperty("rc.throttle.mid", ThrottleMid.getText()
				.toString().replace(",", "."));
		properties.setProperty("rc.throttle.expo", ThrottleExpo.getText()
				.toString().replace(",", "."));
		properties.setProperty("rc.rate",
				RcRate.getText().toString().replace(",", "."));
		properties.setProperty("rc.expo",
				RcExpo.getText().toString().replace(",", "."));
		properties.setProperty("rc.throttle.rate", ThrottleRate.getText()
				.toString().replace(",", "."));

		properties.storeToXML(fos, new Date().toString());

		Toast.makeText(getApplicationContext(),
				getString(R.string.Settingssaved), Toast.LENGTH_SHORT).show();
